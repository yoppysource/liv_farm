...
class PaymentPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    PaymentViewModel model = Provider.of<PaymentViewModel>(context, listen: true);
    MyUser user = Provider.of<LandingPageViewModel>(context,listen: false).user;
    return ModalProgressHUD(
      inAsyncCall: model.isBusy,
      child: IamportPayment(
        appBar: AppBar(
          leading: IconButton(icon: Icon(Icons.arrow_back_ios),onPressed: (){Navigator.pop(context, false);}, ),
          title: Text('결제하기',style: TextStyle(color: Colors.black, fontWeight: FontWeight.w500),),
          elevation: 0,
